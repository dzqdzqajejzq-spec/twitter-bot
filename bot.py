...
# ===================== CONFIG =====================
ACCOUNTS_TO_WATCH = ["mrboogie33", "deadlovesgrave"]  # comptes à surveiller
...
async def send_discord(text, url, username):
    embed = {
        "embeds": [{
            "author": {
                "name": f"@{username}",
            ...
            "footer": {"text": "🔒 Archivé • même si supprimé il sera là"}
        }]
    }
    async with httpx.AsyncClient() as http:
        resp = await http.post(DISCORD_WEBHOOK, json=embed)
        if resp.status_code == 204:
            print(f"✅ Envoyé sur Discord : {url}")
        else:
            print(f"❌ Erreur Discord : {resp.status_code}")
        ...
        try:
            ...
            for entry in feed.entries[:5]:
                ...
                if tweet_id not in seen_ids:
                    ...
                    await send_discord(entry.title, entry.link, username)
